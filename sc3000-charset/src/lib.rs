...
/// Generate a character map for text created on Japanese systems.
pub fn gen_japanese_charmap() -> HashMap<u8, char> {
    HashMap::from([
        ( 0x0d, '\x0a' ),
        ...
        ( 0xe4, '\u{2595}' ), ( 0xe5, '\u{2588}' ), ( 0xe6, '\u{fffd}' ), ( 0xe7, '\u{fffd}' ),
        ( 0xe8, '\u{fffd}' ), ( 0xe9, '\u{fffd}' ), ( 0xea, '\u{259e}' ), ( 0xeb, '\u{25cb}' ),
        ...
        ( 0xf8, '\u{2663}' ), ( 0xf9, '\u{fffd}' ), ( 0xfa, '\u{fffd}' ), ( 0xfb, '\u{fffd}' ),
        ( 0xfc, '\u{fffd}' ), ( 0xfd, '\u{01fbc5}' ), ( 0xfe, '\u{00f7}' ),
    ])
}

/// Generate a character map for text created on 'export' systems.
pub fn gen_export_charmap() -> HashMap<u8, char> {
    HashMap::from([
        ( 0x0d, '\x0a' ),
        ...
        ( 0xcc, '\u{00bf}' ), ( 0xcd, '\u{00a1}' ), ( 0xce, '\u{fffd}' ), ( 0xcf, '\u{00a3}' ),
...
pub fn convert_utf8(source: &[u8], charmap: &HashMap<u8, char>) -> String {
    let mut dest = "".to_string();
    dest.reserve(source.len() * 3);

    for src_byte in source {
        if charmap.contains_key(src_byte) {
            dest.push(charmap[src_byte]);
        } else {
